...
struct Card {
    ...
    func imageName() -> String
    {
        ...
        else if blackActions != nil
        {
            return "\(blackActions!).png"
        }
        else
        {
            return ""
...
var blackActions = ["Wild", "Wild_Draw"]
for i in color
{
    for number in 0...9
    {
        cards.append(Card(color: i, number: number))
    }
    for number in 1...9
    {
        cards.append(Card(color: i, number: number))
    ...
    {
        cards.append(Card(color: i , actions: action ))
    }
